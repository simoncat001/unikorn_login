...
def main() -> None:
    ...
    primary_file, dataset_files = detect_cbf_files(args.input.resolve())
    dataset_dir = primary_file.parent
    cbf_candidates = {primary_file.resolve()}
    for item in dataset_files:
        path_obj = Path(item)
        if path_obj.suffix.lower() == ".cbf":
            cbf_candidates.add(path_obj.resolve())
    metadata = parse_sem_metadata(sorted(cbf_candidates))
    template = load_template(args.template)
    populated = populate_template(template, metadata, dataset_dir, dataset_files, primary_file)

    if args.pretty:
        output_text = json.dumps(populated, ensure_ascii=False, indent=2)
    else:
        output_text = json.dumps(populated, ensure_ascii=False)
    if args.output:
